...
export async function POST(request: Request) {
  try {
    ...
    const html = `<h2>New website inquiry</h2><p><b>Name:</b> ${safe(body.name)}</p><p><b>Phone:</b> ${safe(body.phone)}</p><p><b>Email:</b> ${safe(body.email)}</p><p><b>Inquiry reason:</b> ${safe(body.reason || "—")}</p><p><b>Message:</b> ${safe(body.message || "—")}</p>`;
    const sent = await resend.emails.send({ from, to, subject: "Website inquiry", html }); if (sent.error) throw new Error(sent.error.message);
