...
export function ProductCard({ product }: { product: Product }) {
  return <article className="group overflow-hidden rounded-2xl border border-forest/8 bg-white shadow-[0_12px_45px_rgba(18,45,27,.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(18,45,27,.12)]"><Link href={`/products/${product.slug}`} className="block"><div className="relative aspect-[4/3] overflow-hidden bg-[#f4f0e7]"><Image src={product.image} alt={`${product.name} product`} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-contain p-4 transition duration-700 group-hover:scale-[1.03]" /></div><div className="p-6"><p className="eyebrow">{product.category}</p><h3 className="mt-3 text-2xl font-medium text-leaf">{product.name}</h3><p className="mt-3 min-h-14 text-sm leading-6 text-ink/55">{product.summary}</p><span className="mt-5 inline-block text-xs font-bold uppercase tracking-wider text-gold">View details →</span></div></Link></article>;
}
