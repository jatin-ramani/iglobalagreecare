...
export function HeroCarousel() {
  ...
  return <section className="relative min-h-[760px] overflow-hidden text-white"><Image key={product.slug} src={product.image} alt={product.name} fill priority sizes="100vw" className="object-cover" /><div className="absolute inset-0 bg-linear-to-r from-[#0c2416]/95 via-[#0c2416]/70 to-[#0c2416]/20"/><div className="container-site relative flex min-h-[760px] items-center pt-24"><div className="max-w-3xl"><p className="eyebrow inline-block rounded-full border border-white/30 px-4 py-2 text-white">Indian agricultural exports</p><h1 className="mt-7 text-5xl font-medium leading-[.9] tracking-[-.045em] sm:text-7xl lg:text-8xl">Quality rooted in India.<br/><em className="text-[#e9bb84]">Ready for the world.</em></h1><p className="mt-6 max-w-xl text-base leading-7 text-white/80"><b className="text-white">{product.name}.</b> {product.summary}</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/products" className="rounded-full bg-gold px-6 py-3 text-xs font-bold uppercase tracking-wider">Explore products</Link><Link href="/contact" className="rounded-full border border-white/40 px-6 py-3 text-xs font-bold uppercase tracking-wider">Get a quote</Link><Link href={`/products/${product.slug}`} className="px-2 py-3 text-xs font-bold uppercase tracking-wider text-white/75">View more →</Link></div><div className="mt-10 flex gap-2">{featuredProducts.map((item, index) => <button onClick={() => setActive(index)} aria-label={`Show ${item.name}`} key={item.slug} className={`h-1 rounded-full transition-all ${index === active ? "w-10 bg-gold" : "w-5 bg-white/35"}`} />)}</div></div></div></section>;
}
...
const testimonials = [
  ["Overseas importer", "Communication was clear from product discussion through dispatch planning, which made the sourcing process straightforward."],
  ["Food processor", "The team understood our specification questions and responded with practical options for commercial quantities."],
  ["Domestic distributor", "Reliable follow-up and a broad product range make IGLOBALAGREECARE a useful sourcing partner."],
];
